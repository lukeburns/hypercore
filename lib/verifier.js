...
class Signer {
  constructor(
    ...
    version,
    index,
    { signature = cryptoSpec.DEFAULT_SIGNATURE, publicKey, namespace = caps.DEFAULT_NAMESPACE } = {}
  ) {
    if (!publicKey) throw BAD_ARGUMENT('public key is required for a signer')
    if (!cryptoSpec.isSupportedSignature(signature)) {
      throw BAD_ARGUMENT('Unsupported signature algorithm: ' + signature)
    }
  ...
  verify(batch, signature) {
    return cryptoSpec.verify(batch.signable(this._ctx()), signature, this.publicKey, this.signature)
  }

  sign(batch, keyPair) {
    return cryptoSpec.sign(batch.signable(this._ctx()), keyPair, this.signature)
  }
}
...
class CompatSigner extends Signer {
  ...
  verify(batch, signature) {
    return cryptoSpec.verify(
      batch.signableCompat(this.legacy),
      signature,
      this.publicKey,
      this.signature
    )
  ...
  sign(batch, keyPair) {
    return cryptoSpec.sign(batch.signableCompat(this.legacy), keyPair, this.signature)
  }
}
...
module.exports = class Verifier {
  constructor(
    ...
    this.hash = manifest.hash || cryptoSpec.DEFAULT_HASH
  ...
  static defaultSignerManifest(publicKey) {
    return {
      version: 1,
      hash: cryptoSpec.DEFAULT_HASH,
      allowPatch: false,
      quorum: 1,
      signers: [
        {
          signature: cryptoSpec.DEFAULT_SIGNATURE,
  ...
  static createManifest(inp) {
    ...
    if (inp.hash && inp.hash !== cryptoSpec.DEFAULT_HASH) {
      throw BAD_ARGUMENT('Only ' + cryptoSpec.DEFAULT_HASH + ' hashes are supported')
    }

    if (inp.prologue) {
      if (
        !(
          b4a.isBuffer(inp.prologue.hash) && inp.prologue.hash.byteLength === cryptoSpec.HASH_BYTES
        ) ||
    ...
    if (inp.linked && inp.linked.length) {
      ...
      for (const key of inp.linked) {
        if (!(b4a.isBuffer(key) && key.byteLength === cryptoSpec.CORE_KEY_BYTES)) {
...
function parseSigner(signer) {
  ...
  return {
    signature: signer.signature || cryptoSpec.DEFAULT_SIGNATURE,
...
function validateSigner(signer) {
  ...
  if (signer.signature && !cryptoSpec.isSupportedSignature(signer.signature)) {
    throw BAD_ARGUMENT('Unsupported signature algorithm: ' + signer.signature)
  }
}
