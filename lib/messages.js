...
const namespaceBuffer = fixedBuffer(DEFAULT_NAMESPACE.byteLength, 'namespace')
const hashBuffer = fixedBuffer(cryptoSpec.HASH_BYTES, 'hash')
const signerPublicKeyBuffer = fixedBuffer(cryptoSpec.PUBLIC_KEY_BYTES, 'signer publicKey')
const signerSignatureBuffer = fixedBuffer(cryptoSpec.SIGNATURE_BYTES, 'signer signature')
const coreKeyBuffer = fixedBuffer(cryptoSpec.CORE_KEY_BYTES, 'core key')
...
const hashes = {
  ...
  decode(state) {
    ...
    if (n === 0) return cryptoSpec.DEFAULT_HASH
...
const signatures = {
  ...
  encode(state, m) {
    if (m === cryptoSpec.DEFAULT_SIGNATURE) {
      c.uint.encode(state, 0)
      return
    }
  ...
  decode(state) {
    const n = c.uint.decode(state)
    if (n === 0) return cryptoSpec.DEFAULT_SIGNATURE
...
const signer = {
  preencode(state, m) {
    ...
    namespaceBuffer.preencode(state, m.namespace)
    signerPublicKeyBuffer.preencode(state, m.publicKey)
  },
  encode(state, m) {
    signatures.encode(state, m.signature)
    namespaceBuffer.encode(state, m.namespace)
    signerPublicKeyBuffer.encode(state, m.publicKey)
  },
  decode(state) {
    return {
      signature: signatures.decode(state),
      namespace: namespaceBuffer.decode(state),
      publicKey: signerPublicKeyBuffer.decode(state)
    }
  }
}
...
const prologue = {
  preencode(state, p) {
    hashBuffer.preencode(state, p.hash)
    c.uint.preencode(state, p.length)
  },
  encode(state, p) {
    hashBuffer.encode(state, p.hash)
    c.uint.encode(state, p.length)
  },
  decode(state) {
    return {
      hash: hashBuffer.decode(state),
...
const manifestv0 = {
  preencode(state, m) {
    ...
    if (m.prologue && m.signers.length === 0) {
      hashBuffer.preencode(state, m.prologue.hash)
      return
    }
  ...
  encode(state, m) {
    ...
    if (m.prologue && m.signers.length === 0) {
      c.uint.encode(state, 0)
      hashBuffer.encode(state, m.prologue.hash)
      return
    }
...
const fixed32Array = c.array(coreKeyBuffer)
...
const node = {
  preencode(state, n) {
    ...
    hashBuffer.preencode(state, n.hash)
  },
  encode(state, n) {
    c.uint.encode(state, n.index)
    c.uint.encode(state, n.size)
    hashBuffer.encode(state, n.hash)
  },
  decode(state) {
    return {
...
wire.handshake = {
  preencode(state, m) {
    ...
    hashBuffer.preencode(state, m.capability)
  },
  encode(state, m) {
    c.uint.encode(state, m.seeks ? 1 : 0)
    hashBuffer.encode(state, m.capability)
  },
  decode(state) {
    const flags = c.uint.decode(state)
    return {
      seeks: (flags & 1) !== 0,
      capability: unslab(hashBuffer.decode(state))
    }
  }
}
...
oplog.header = {
  preencode(state, h) {
    ...
    coreKeyBuffer.preencode(state, h.key)
  ...
  encode(state, h) {
    ...
    coreKeyBuffer.encode(state, h.key)
  ...
  decode(state) {
    ...
    return {
      external: null,
      key: coreKeyBuffer.decode(state),
...
const multisigInput = {
  preencode(state, inp) {
    ...
    signerSignatureBuffer.preencode(state, inp.signature)
    c.uint.preencode(state, inp.patch)
  },
  encode(state, inp) {
    c.uint.encode(state, inp.signer)
    signerSignatureBuffer.encode(state, inp.signature)
    c.uint.encode(state, inp.patch)
  },
  decode(state) {
    return {
      signer: c.uint.decode(state),
      signature: signerSignatureBuffer.decode(state),
...
function fixedBuffer(length, name) {
  return {
    preencode(state, buffer) {
      if (buffer.byteLength !== length) throw new Error('Invalid ' + name + ' length')
      state.end += length
    },
    encode(state, buffer) {
      if (buffer.byteLength !== length) throw new Error('Invalid ' + name + ' length')
    ...
    decode(state) {
      ...
      if (end > state.end) throw new Error('Invalid ' + name + ' length')
